...
def check_duplicate_claims(user_id: int, db: Session) -> dict:
    ...
    Check if the user has filed another claim in the past 24 hours.
    Real disruptions are usually single events — multiple claims same day = suspicious.
    """
    ...
    if recent_claims >= 2:
        return {
            "flagged": True,
            "reason": f"User has {recent_claims} claims in past 24 hours (limit: 1)",
            "score": 0.5
        }
